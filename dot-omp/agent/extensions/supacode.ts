 ...
 *
 * Hook event mapping:
 *   omp agent_start      → busy = 1       (UserPromptSubmit equivalent)
 *   omp agent_end        → busy = 0       (Stop equivalent)
 *                        → notification with last_assistant_message
 *   omp session_shutdown → busy = 0       (SessionEnd equivalent)
 */
 ...
 * Times out after 1 s (omp serializes hook callbacks, so a stalled
...
function sendNotification(env: SupacodeEnv, payload: HookPayload): Promise<void> {
	const header = `${env.worktreeId} ${env.tabId} ${env.surfaceId} omp\n`;
