...
- `GITHUB_TOKEN` is provided by the 1Password CLI plugin. It is directory-aware:
  - **Outside `~/dev/personal/`**: authenticates as the work account (`TrevorSmith-Wellsky`) — use `github.com` directly for remotes and `gh` commands
  - **Inside `~/dev/personal/`**: authenticates as the personal account (`tvrmsmith`) — use `github-personal` SSH host alias for remotes, and `gh auth switch --user tvrmsmith` before `gh` commands (PRs, issues, etc.)
- When running `gh` commands against work repos from inside `~/dev/personal/`, `cd` to a non-personal directory first (e.g. `cd ~/dev && gh repo view ...`)
