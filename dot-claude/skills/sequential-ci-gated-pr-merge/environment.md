...
- **macOS bash:** only `/bin/bash` (3.2) may exist; Homebrew bash may be absent. `pr-monitor.sh` uses `#!/bin/bash` shebang, stays 3.2-compatible. Some other tooling (e.g. local `repo` verify) needs `PATH=/opt/homebrew/bin:$PATH` — set for those, not for monitor.
