...
- **Never skip check applying to service PR edits — even when non-required.** `mergeStateStatus: UNSTABLE` means *required* gate satisfied, but non-required check can still be pending/failing. Before merge past any such check, map it to paths changed in PR (`gh pr diff <pr> --name-only`): if it exercise service/area PR touched (e.g. PR edits `hospice-intake` and `hospice-intake-api` pending, or edits web-bff and `hospice-web-bff-api` pending), **wait for green** — no merge past. Merge past not-green non-required check only when genuinely unrelated to PR changes (see systemic-failure rule below).
...
2. **Monitor:** run `pr-monitor.sh <pr> <repo>` in background. Poll every 30s, exit with `RESULT=` line + exit code. Act on result:
   ...
   - `RESULT=UNSTABLE` (exit 15) → required checks green but monitor listed non-required checks not yet green. Map each to PR changed paths (`gh pr diff <pr> --name-only`): if one apply to service PR edited (e.g. `hospice-web-bff-api` when PR touch web-bff), **poll to green** before merge — no merge past. Merge past not-green non-required check only when systemic (also failing on `main`) AND unrelated to changed paths. Re-check `mergeStateStatus`/main-HEAD immediately before merge, since wait may have let main advance.
   ...
   - `RESULT=CHECK_FAILED` (exit 20) → required check failed → STOP, report to user.
...
- **macOS bash:** only `/bin/bash` (3.2) may exist; Homebrew bash may be absent. `pr-monitor.sh` use `#!/bin/bash` shebang, stay 3.2-compatible. Some other tooling (e.g. local `repo` verify) need `PATH=/opt/homebrew/bin:$PATH` — set for those, not for monitor.
...
- **`mergeStateStatus` values you see:**
  ...
  - `UNSTABLE` — required checks satisfied (PR *is* mergeable) but non-required check pending/failing. Do NOT auto-merge. Map each not-green non-required check to PR changed paths: wait on any applying to edited service; merge only once those green (systemic/unrelated failures excepted, per policy rules above).
