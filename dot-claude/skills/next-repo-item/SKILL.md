---
name: next-repo-item
description: Pick the next ready beads item by dependency order and start work on it — sets up an isolated workspace via a chosen orchestrator (superset, orca, or sbx) and kicks off implementation. Handles ready-for-agent (with or without brief) and ready-for-human states. Redacted.Repo only.
...
## Repo gate — Redacted.Repo only

Global skill, but only applies to the **Redacted.Repo** repo (assumes `repo`, beads, hospice, `docs/agents/triage-labels.md`). Before anything else, confirm the current repo is Redacted.Repo:

- `git config --get remote.origin.url` contains `Redacted.Repo`, OR the repo toplevel / git-common-dir basename is `Redacted.Repo`.

If not Redacted.Repo, STOP and report: "next-repo-item only runs in the Redacted.Repo repo." Do not proceed.

## Invocation

- `/next-repo-item` — global scope, next most-unblocking item.
- `/next-repo-item <epic-id>` — restrict to that epic's subtree.
- `/next-repo-item --spawn` — force a new workspace.
- `/next-repo-item --in-place` — reuse current worktree (requires clean + unclaimed).
- `/next-repo-item --orchestrator <superset|orca|sbx>` (aliases `--superset`,
