---
name: next-repo-item
description: Pick the next ready beads item (dependency-ordered) and start work via a chosen orchestrator (superset, orca, or sbx). Redacted.Repo only.
...
Global skill, only applies to **Redacted.Repo** repo (assumes `repo`, beads, hospice, `docs/agents/triage-labels.md`). First, confirm current repo is Redacted.Repo:

- `git config --get remote.origin.url` contains `Redacted.Repo`, OR repo toplevel / git-common-dir basename is `Redacted.Repo`.
...
- `/next-repo-item <epic-id>` — restrict to that epic subtree.
- `/next-repo-item --spawn` — force new workspace.
