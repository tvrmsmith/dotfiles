...
issue for the **Redacted.Repo** repo and starts work on it — drafting an agent
...
Invoke explicitly with `/next-repo-item` (model auto-invocation is disabled).

## Scope

Global skill (lives in `~/.claude/skills`, so it loads in every project) but
**gated to Redacted.Repo**. On invocation it verifies the current repo is
Redacted.Repo (origin URL / repo basename) and stops otherwise. It assumes the
`repo` CLI, a beads issue DB, the hospice product, and the personal triage
overlay in `docs/agents/triage-labels.md`.

## Invocation

- `/next-repo-item` — global scope; next most-unblocking ready item.
- `/next-repo-item <epic-id>` — restrict candidates to that epic's subtree.
- `/next-repo-item --spawn` — force a new workspace.
- `/next-repo-item --in-place` — reuse the current worktree (requires clean +
  unclaimed).
