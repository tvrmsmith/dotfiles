...
- `dot-local/bin/gh` shims the real `gh`: reads use a read-only token pulled from the login keychain per invocation, keyed by GitHub resource owner (`security find-generic-password -s gh-readonly -a <owner>`) because a fine-grained PAT has exactly one owner, writes re-exec through `op plugin run -- gh` and prompt. `GH_SHIM_EXPLAIN=1 gh <args>` prints the routing decision and runs nothing; `tests/gh-shim.bats` covers the classifier. Provision the tokens with `extras/gh-readonly-tokens.sh` (rerunnable). Sourcing `~/.config/op/plugins.sh` again would shadow the shim, since an alias beats PATH
