...
- Agent sandboxing lives in `dot-agent-sandboxing/` (directory of `*.zsh` files glob-sourced by shell rc; split into `00-shared.zsh`, `10-nono.zsh`, `20-dsbx.zsh`) and `dot-claude/sandbox/` (Dockerfiles + compose). Two stacks: `nono-*` ([nsjail-based](https://nono.sh/docs/)) and `dsbx-*` ([Docker `sbx`](https://docs.docker.com/ai/sandboxes/)). `sbx` runs microVMs in an embedded containerd — host `docker ps`/`exec`/`logs` won't see them; use `sbx ls`/`exec`/`run`, or `docker -H unix://$_DSBX_SBXD_SOCK ...` for the inner daemon. Edit those files directly — do not duplicate aliases/helpers elsewhere
- When invoking `dsbx-*` from inside a coding agent's bash tool, use `--print`/`-p` (e.g. `dsbx-omp -p "explain X"`). It runs the agent's print mode via `sbx exec -i`, never attaches a TTY. Without it, `sbx run` grabs `/dev/tty` from the parent and on exit EPIPEs the parent's renderer, wedging the host terminal.
